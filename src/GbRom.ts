...
const ramMap = new Map<number, MemoryInfo>(
  banksArray.map(([key, nBanks]) => [
    key,
    { banks: nBanks, size: 8192 * nBanks },
  ])
);

const romMap = new Map<number, MemoryInfo>(
  range(9).map((i) => [i, { banks: 2 << i, size: 32768 << i }])
);
...
export class Logo {
  ...
  get pixelArray(): Array<boolean> {
    const pixelArray = Array<boolean>(384).fill(false);
  ...
  eachPixel(func: (x: number, y: number, value: boolean) => void) {
    let byteI = 0;
  ...
  get ascii() {
    const lines = Array.from(Array(8), () => " ".repeat(48));
...
class Header {
  ...
  set titleMin(title: string) {
    this._titleMinBuffer.write(padNull(title, 11), 0, 11, "ascii");
  }
  ...
  set title(title: string) {
    this._titleBuffer.write(padNull(title, 15), 0, 15, "ascii");
  }
  ...
  set titleMax(title: string) {
    this._titleMaxBuffer.write(padNull(title, 16), 0, 15, "ascii");
  }
  ...
  get licensee(): string | undefined {
    if (this.licenseeCodeOld === 0x33) {
      return licenseeNewMap.get(this.licenseeCodeNew);
    } else {
      return licenseeOldMap.get(this.licenseeCodeOld);
    }
  }
...
export class GbRom extends BaseRom {
